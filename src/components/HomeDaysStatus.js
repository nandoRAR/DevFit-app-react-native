...
export default (props) => {
    ...
    if (!props.workoutDays.includes(thisDate.getDay())) {
        ...
        (props.dailyProgress.includes(dFormated)) ? isDone = true : isDone = false;

    }
    (thisDate.getTime() == today.getTime()) ? isToday = true : isToday = false;

    const setDone = () => {
        props.addProgress(dFormated);
    }
    const setUnDone = () => {
        props.dellProgress(dFormated);
    }
                ...
                {!dayOff && !isFuture && isDone &&
                    <>
                        ...
                        <DefaultButton onPress={setUnDone} underlayColor='#4AC34E' bgcolor="#4AC34E" style={{ marginTop: 20 }}>
                            <ButtonText>DESMACAR</ButtonText>
                        </DefaultButton>
                    </>
                }
